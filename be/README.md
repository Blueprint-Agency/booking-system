...
| `npm run check` | Run every `src/**/*.test.ts` and `tools/**/*.test.ts` self-check (Node's built-in test runner + tsx; no test framework). Exits non-zero if any assertion fails. |
...
| `npm run mindbody:download` / `npm run mindbody` | Mindbody migration tooling in `tools/mindbody/` (download, transform) — not part of the app; see `../docs/md/mindbody-import.md` |
