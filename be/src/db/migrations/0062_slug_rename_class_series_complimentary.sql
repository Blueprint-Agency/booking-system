...
--> statement-breakpoint
ALTER TABLE "client_packages" ADD COLUMN "complimentary" boolean DEFAULT false NOT NULL;--> statement-breakpoint
ALTER TABLE "classes" ADD COLUMN "series_id" uuid;--> statement-breakpoint
ALTER TABLE "former_slugs" ADD CONSTRAINT "former_slugs_renamed_tenant_id_tenants_id_fk" FOREIGN KEY ("renamed_tenant_id") REFERENCES "public"."tenants"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "former_slugs_renamed_tenant_id_idx" ON "former_slugs" USING btree ("renamed_tenant_id");--> statement-breakpoint
...
CREATE INDEX "classes_series_starts_idx" ON "classes" USING btree ("tenant_id","series_id","starts_at");--> statement-breakpoint
ALTER TABLE "client_packages" ADD CONSTRAINT "client_packages_complimentary_free" CHECK (NOT "client_packages"."complimentary" OR ("client_packages"."amount_paid_sgd" = 0 AND "client_packages"."stripe_payment_intent_id" IS NULL));