...
/**
 * Write every email template. UPSERT on slug, so a copy change ships with the
 * deploy (`deploy-be.yml` runs `db:seed` after `db:migrate`).
 *
...
export async function seedEmailTemplates(db: PostgresJsDatabase<typeof schema>) {
  ...
  for (const t of templates) {
    await db.execute(sql`
      ...
      VALUES (${t.slug}, ${t.subject}, ${t.bodyHtml})
