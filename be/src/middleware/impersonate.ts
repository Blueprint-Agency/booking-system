...
/**
 * Admin impersonation: when an admin sends `x-impersonate-staff-id`,
 * the request is treated as if performed by that staff for service-layer
 * decisions, while audit_log records both the actor (admin) and impersonatedBy.
 *
 ...
 * it is refused with the same 403 as a non-admin: whether the id exists
