...
export async function listMerch(opts: { includeArchived: boolean }): Promise<MerchRow[]> {
  return db
    .select()
    .from(merch)
    .where(opts.includeArchived ? undefined : isNull(merch.archivedAt))
    .orderBy(asc(merch.title))
}

export async function getMerch(id: string): Promise<MerchRow> {
  const [row] = await db.select().from(merch).where(eq(merch.id, id)).limit(1)
...
export async function createMerch(input: {
  ...
  const [row] = await db.insert(merch).values(input).returning()
  return row!
}
...
export async function updateMerch(
  ...
  await getMerch(id) // 404 if missing
  const [row] = await db.update(merch).set(patch).where(eq(merch.id, id)).returning()
  return row!
}
...
export async function setMerchImage(input: {
  ...
  const row = await getMerch(input.id)
