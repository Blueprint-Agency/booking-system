...
**A Tenant with no row here takes no online payments** (#293, `be/docs/adr/0007-every-studio-sells-on-its-own-account.md`): its checkouts are refused `409 payments_not_configured` before a Purchase opens, and only $0 purchases — which never reach the provider — go through. There is no platform account to fall back to.
...
- **Each studio charges on its own account, and only there (#100, #293).** Every provider call goes through one Tenant-bound accessor, `stripeForTenant` (`be/src/lib/stripe.ts`), and `providerAccountForTenant` is the single place that answers which account it is on: the studio's own credentials (§4i `tenant_payment_credentials`), or `null` — no online payments, refused `payments_not_configured`. No Stripe key lives in the environment. The member app reads `GET /public/online-payments` before offering a paid buy button. **No statement descriptor suffix** is sent: the studio's own statement already carries its name, and the 22-character limit is measured against a prefix this platform cannot read. A Refund is made against the payment's own account (`stripeForProviderAccount`); a payment recorded against the platform's former account (`provider_account_id` null) is refused `payment_on_platform_account` and refunded from the Stripe dashboard. See `be/docs/adr/0007-every-studio-sells-on-its-own-account.md`.
- **Webhooks, one endpoint per studio.** `/api/v1/webhooks/stripe/{slug}` is a studio's own (the platform account's shared `/api/v1/webhooks/stripe` is gone, #293): its account signs with its own secret, so the studio has to be settled **before** the signature can be checked, and the URL is the only part of a delivery fixed before the body is read. Exactly one secret is tried — no fallback, no sweep of every studio — and the tenant the URL named is carried into the handler, which refuses any event whose body routes elsewhere (`refuseWrongTenant`). Every refusal on that endpoint is the same flat 400, so a URL cannot be used to enumerate which studios take their own money.
