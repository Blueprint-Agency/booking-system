...
- **One bucket** (`R2_BUCKET_NAME`, optional in env like the rest of the storage settings), served by `R2_PUBLIC_URL`. It holds workshop covers, instructor photos **and** instructor medical certificates.
- **Everything in it is public.** An object is readable by anyone who holds its key, with no expiry. There is no private bucket: `R2_PRIVATE_BUCKET_NAME` existed briefly and was removed. Read this before you put anything else in here.
  - For medical certificates the key is the only protection, which is why `medicalCertKey` is `medical-certificates/{instructorId}/{requestId}.{ext}` — two UUIDs — and why no read path serialises it. That is obscurity, not access control. Splitting the certificates back into a bucket with public access disabled is the fix if this is ever judged insufficient.
...
- **Medical certificates — server-side upload (implemented).** The file is POSTed to the API as multipart, field `file` (`POST /portal/instructor/leave/:id/certificate`) → Hono's `bodyLimit` refuses an oversized body before it is buffered at all → the service validates the declared content type and the *real* byte length against the allow-list (`image/jpeg`, `image/png`, `application/pdf`; 5 MB) → `lib/r2.ts#putObject` writes the object → the key is written to `leave_requests.medical_cert_r2_key` only once the object is safely in the bucket.
