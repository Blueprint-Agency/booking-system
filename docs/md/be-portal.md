...
| DELETE | `/clients/:id/permanently` | admin | **Delete permanently** (#144) — on the member's request; blocking stays the reversible option. Every row `MEMBER_TABLES` finds for them is deleted, except the studio's accounts (payments and refunds, package sales, Promo Code uses, merch sales), which stay with the member's identity removed; each table's `erase` steps say which. Ends their sessions at this studio. Their sign-in account is deleted only when no other studio still has a member row for it, and the response does not say which happened. `{ deleted: true }`. Logs `member_deleted` with **no subject**; the audit row records the route pattern, not the id. A second delete, or another studio's member, is 404 `client_not_found`. What is kept, why, and for how long: [`member-data-retention.md`](member-data-retention.md). |
...
| POST | `/clients/:id/email` | admin | `{ email }` — change the member's email (#176). Trimmed and lower-cased; updates `clients.email` and re-links `clients.auth_user_id` to the client-pool account for the new address (created or reused through `ensureAuthUser`) in one transaction, then ends the member's sessions at this studio. A newly created account has no password, so the member's first sign-in there mails them the set-password link (#173); the old account is left alone — it may be the same person at another studio. 409 `email_in_use` when another member of this studio uses it, 400 `email_unchanged`. Audited as `client_email_changed` with the old and new address, and `sessions_revoked` in `auth_events`. **No email is sent by the change itself.** |
...
- `POST /public/staff/sign-in-step { email }` → `{ next: 'password' }` when the address has a staff password and has no live invitation here; else `{ next: 'link_sent' }`. For `link_sent` a set-password link (Better Auth's reset, landing on this portal's `/login`) is mailed if the address is active staff here with no password, or pending here with an unexpired invitation, and nothing otherwise — the answer is the same. A person who is already staff at another studio has one password, so the pending case is what lets them in as this studio's first admin. The step is budgeted per address and per email (429 `too_many_requests`); links are budgeted per email too, and a spent link budget still answers `link_sent`, so no refusal marks an address as staff.
