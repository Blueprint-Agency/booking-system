...
|---|---|---|---|
| GitHub env **variable** names | `FRONTEND_URLS`, `PLATFORM_ADMIN_EMAIL`, `PORT`, `STRIPE_STATEMENT_DESCRIPTOR_PREFIX` | same four | Match. **After #293 deploys:** delete `STRIPE_STATEMENT_DESCRIPTOR_PREFIX` from both — nothing reads it. |
| GitHub env **secret** names | `BETTER_AUTH_SECRET`, `DB_APP_PASSWORD`, `DB_NAME`, `DB_PASSWORD`, `DB_USER`, `IMPERSONATION_SECRET`, `R2_*` ×5, `RESEND_API_KEY`, `RESEND_WEBHOOK_SECRET`, `SENTRY_DSN`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` | same sixteen | Match. **Gap:** `SENTRY_DSN` is dead in both since Sentry was removed — delete from both. **After #293 deploys:** delete `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` from both — nothing reads them — and confirm `PAYMENT_CREDENTIALS_KEY` is set in both, which the deploy now requires. |
...
**`PAYMENT_CREDENTIALS_KEY` seals a studio's own payment credentials.** Every studio charges on **its own** payment-provider account, and only there (#100, #293; `be/docs/adr/0004-tenant-supplied-payment-credentials.md`, `be/docs/adr/0007-every-studio-sells-on-its-own-account.md`): it supplies its secret key, the platform creates the studio's webhook endpoint on that account with it (#294), the super portal stores the key and the endpoint's signing secret encrypted, and every call on that studio's behalf is made against that account. This variable is the encryption key — base64, exactly 32 bytes, `openssl rand -base64 32`.

It is **required in both Environments**; the deploy refuses to proceed without it. Unset, the super portal answers `503 secret_storage_unavailable` rather than storing anything in the clear, so no studio can be given credentials — and **a studio with no credentials takes no online payments at all**: its checkouts are refused `payments_not_configured` and the member app shows "This studio isn't taking online payments yet." in place of a paid buy button ($0 purchases still go through). What it must not be is *changed*: rotating it orphans every stored credential, and each studio's have to be entered again — there is deliberately no second key to fall back to.

A studio's charges carry **no statement descriptor suffix**: its own account's statement already carries its name, set in that account's Stripe settings.
...
**Replacing a key** creates and stores the new endpoint first, then deletes the old one, so there is never a window with no working endpoint. **Removing credentials** ("Stop online payments") deletes the studio's endpoint on its account. Both deletions are best effort: if Stripe cannot be reached the credentials change still happens, and the log line `payment webhook endpoint not deleted` names the `accountId` and `webhookEndpointId` to delete by hand in the studio's Stripe dashboard. A save that fails after the endpoint was created deletes it again, so a retry never leaves two.

**A restricted key** must have the **Webhook Endpoints: Write** permission; without it the form says so, `400 provider_key_lacks_webhook_permission`, and nothing is created or stored. A standard secret key always can. Saved keys still can't be viewed, only replaced. There is no shared endpoint any more — `/api/v1/webhooks/stripe` went with the platform account (#293).

**Rolling out #293 (the platform account stops being used).** Before the deploy: confirm production has no payments recorded against the platform account (`stripe_payments.provider_account_id IS NULL` — the platform's live account was never activated, so there should be none; staging's are test payments and may be abandoned), and enter each selling studio's own credentials **first** — the moment it ships, a studio without them stops taking online payments. After the deploy: delete `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` and `STRIPE_STATEMENT_DESCRIPTOR_PREFIX` from both GitHub Environments, and delete the platform account's webhook endpoints in the Stripe dashboard. A payment still recorded against the platform account is refused a Refund in the app (`payment_on_platform_account`) and has to be refunded from the Stripe dashboard.
...
| `PAYMENT_CREDENTIALS_KEY` | be | GH env → host `.env` | Backend dev | Never, short of a leak | **No overlap and no second key:** rotating it orphans every studio's stored payment credentials, so every studio stops taking online payments until its credentials are entered again in the super portal. A studio's own Stripe keys are rotated by entering new ones there, not here. |
...
- be, GH env vars: `PORT`, `PLATFORM_ADMIN_EMAIL`, `FRONTEND_URLS`, `LOG_LEVEL`.
