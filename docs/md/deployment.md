...
- `env vars` (set in **both** Environments): `PORT`, `TENANT_ORIGIN_PATTERNS`, `PLATFORM_ADMIN_EMAILS` (optional), `MAIL_FROM_EMAIL` (required), `MAIL_FROM_PORTAL_EMAIL` / `MAIL_FROM_NAME` (optional) — the platform's envelope identity; see below
