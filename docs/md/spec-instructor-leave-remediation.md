...
Three problems have teeth. **Leave emails can fail silently in production** — three new catch blocks flatten the error to a string instead of logging the full error object, so an instructor whose leave was approved but never told, and an admin who never learned a class was cancelled, produce no trace anyone will see. **Two instructors submitting leave at the same moment can both be accepted** even when together they exceed the allowance, because the submission reads the existing rows and inserts without a transaction — the "pending counts against your balance" rule that exists precisely to prevent over-commitment has a hole in it. And **revoking approved leave erases who approved it**, overwriting the decision columns with the revoker's identity.
...
13. ~~As a developer, I want the private bucket documented as in use rather than reserved, so that nobody plans a feature around it being free.~~ **Superseded (2026-08-17) — see the Documentation alignment section below.** The private bucket this story asks to document was itself removed at the owner's direction before this remediation shipped. There is no private bucket to plan around; the opposite story now holds.
...
21. As a developer, I want the ticket and spec wording for the allowance screen corrected to match the gated policy surface, so that the next reader does not treat a deliberate gate as a bug.
...
- The backend architecture document's object-storage section is corrected to describe the implemented flow: the API receives the file, validates type and size server-side, and writes to storage; retrieval is a short-lived signed GET.
- ~~The private bucket is documented as in use for medical certificates, not reserved.~~ **Superseded (2026-08-17).** This was correct about the code as it stood on 2026-08-10: a separate private bucket (`R2_PRIVATE_BUCKET_NAME`) existed and certificates landed in it. It does not describe the code any more. The owner removed that bucket shortly after this remediation was written — confirmed again on 2026-08-17 — and certificates (now **Supporting Documents**, renamed with the study-leave ticket) live in the one existing public bucket, named by `R2_BUCKET_NAME`, exactly like every other object in it. The two UUIDs baked into the storage key are the only thing standing between a public URL and a document, and that is the accepted, deliberate protection — not a stopgap awaiting a private bucket. `backend-architecture.md` §6c and `spec-instructor-leave.md` "Supporting Document upload" carry the current, accurate description; nothing here should be read as still describing a second bucket.
- **This resolves a contradiction, and the resolution is a real decision, not a formality — as of when it was written.** The documented presigned-PUT flow was written for public imagery. Medical certificates were, at the time, validated at a trust boundary the server controls and landed in a private bucket, and server-side upload avoided configuring cross-origin access on a bucket holding health documents. The implemented flow was kept and the document corrected to match. The private-bucket half of that reasoning no longer applies — see the superseded note above — but the server-side-upload-and-signed-GET half still describes the shipped flow, private bucket or not. Public imagery uploads, if they are ever built, may still use the presigned flow — the document should say which applies where rather than describing one flow for everything.
- The leave spec and ticket 02's acceptance criterion are corrected to say the allowances are set on the policy screen, by the role that screen is gated to. The policy surface is gated by existing design across every other setting on it; carving out an exception for two integers would make these the odd ones out. The code is right and the wording was wrong.
  - **Superseded (2026-08-12), and worth reading in that order.** The correction above was right about the code as it then stood: two studio-wide allowance integers on the gated policy screen, and a ticket that described them as an admin's to set. Both integers have since gone. The yearly figure is now **Assigned Days**, held per instructor and set on the staff profile by an **admin** — so the person who approves the leave is now also the person who sets the number, which is the thing the earlier correction ruled out. What survived is the reasoning, not the conclusion: the policy screen is still gated across every setting on it, and the one leave field left there — the studio-wide carry-over cap — is gated for exactly the reason given above.
  - **Superseded again (2026-09-16).** The portal now has two staff roles, admin and instructor, and the policy screen's gate is admin — see `be/docs/adr/0006-two-staff-roles.md`. The gate still exists; it is simply the same role that approves the leave. See `spec-instructor-leave-pools.md` and `be/docs/adr/0001-per-instructor-leave-pools-with-carry-over.md`.
