...
**Eligibility is a notice, not a gate.** The studio's rule — once a member has attended a class on the plan, no refund — is enforced by showing it. The button is always present; when the purchase is not **Untouched** a notice sits above it ("3 classes used (attended or no-show) since 12 June" — the count includes no-shows, so the sentence says so) and the admin may refund anyway. A gate was rejected because it cannot gate anything while the dashboard exists, and it would strand the genuine cases the studio does want to refund.

**Between the button and the webhook the Refund is processing** (#275). The payment row still reads `succeeded` until `charge.refunded` lands, so each payment the provider accepts a Refund for is stamped `refund_requested_at`, and the portal shows "Refund processing" and withholds the button while every held payment is stamped — a second press is refused as `refund_processing` and never reaches the provider. A stamp older than a day is no longer believed — the Refund failed at the provider after accepting it, or its webhook was lost — so the button comes back rather than being stranded behind "processing" for good. A Refund over several payments that the provider refuses part-way is not thrown as an error, because the request's transaction would roll back the stamps recording what did go back: it answers `complete: false` with how many payments the provider accepted, the admin is told "Some of this refund may have gone through. Check Stripe before you try again.", and the purchase shows "Refund incomplete" — still one Refund of the whole, just unfinished, and it stays so after the first payment's webhook lands — with the button still offered: pressing it returns only the payments not yet asked for. The dialog states what the Refund does before the admin commits: the amount going back (and whether the Cross-Location Add-On goes with it or, bought separately, simply ends), that the package stops covering bookings, how many upcoming bookings are cancelled, and which Promo Code is freed.
