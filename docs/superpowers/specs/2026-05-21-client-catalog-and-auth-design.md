...
### Approach
Reuse the existing `listSchedule()` in `be/src/services/schedule/timetable.ts` (already joins class type, instructor, location, room and counts confirmed bookings) behind a thin **client serializer**, rather than writing a parallel query. This keeps admin and client views from drifting — same source of truth for "what's scheduled" and "how full it is".
...
- `GET /public/catalog/classes` — zod query: `location_id?`, `instructor_id?`, `class_type_id?`, `from?` (ISO date), `to?` (ISO date). Default window today → +28 days; cap range at 90 days. Returns active (`lifecycle='active'`) instances in window. Per instance:
  `id, class_type {id, name}, instructor {id, name}, location {id, name}, room {id, name} | null, starts_at, ends_at, credit_cost, capacity_online, booked_count, spots_left, difficulty, lifecycle`.
...
- `/classes` page: remove `sessions.json` + mock-state reads; keep the existing calendar/filter UI (instructor, level, location, date) and feed it live data. Signed-in users see `is_booked` badges. "Book" button calls the still-stubbed `POST /me/bookings/class` (out of scope; surfaced as disabled/"coming soon" affordance to avoid a broken click).
