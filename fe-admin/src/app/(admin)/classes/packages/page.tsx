...
export default function ClassPackagesPage() {
  ...
  const products = state.products.filter((p) => p.creditType === "class");

  return (
    <div className="space-y-6">
      <PageHeader
        title="Class packages"
        description="Drop-in, class packs, and memberships. Credits redeem on regular classes."
      ...
      <div className="overflow-hidden rounded-xl border border-ink/10 bg-white">
        ...
        {products.length === 0 && (
          <div className="p-8 text-center text-sm text-ink/50">No class packages yet.</div>
        )}
      </div>
