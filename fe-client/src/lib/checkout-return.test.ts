...
test("PAY-20 a cancelled checkout says the member hasn't been charged", () => {
...
test("PAY-20 a cancelled resumed purchase says so, and that the balance is still there", () => {
...
test("PAY-20 an ordinary visit shows no notice", () => {
...
test("PAY-21 a confirmed sync is a payment, a pending one is not", () => {
...
test("PAY-21 a sync that failed or answered nothing we know is pending, never successful", () => {
...
test("PAY-22 a $0 grant has no payment session, so it is free, not paid", () => {
  assert.equal(confirmationOutcome(null, "failed"), "free");
  assert.equal(confirmationOutcome(null, { ok: true, body: { status: "granted" } }), "free");
});

test("PAY-22 only a confirmed payment is ever called successful", () => {
