 ...
 *   dev.reservetoday.app        acme.dev.reservetoday.app
 *   reservetoday.app            acme.reservetoday.app
 *
 ...
 *
 * `portal` is the one that bites: on the production client root
 * `reservetoday.app`, `portal.reservetoday.app` is fe-portal's own hostname and
 * must not be read as a studio called "portal".
 */
 ...
 * of the comparison, so `localhost:3000` and `localhost:3005` are the same root
 * domain — a dev server on another port must not stop resolving Tenants.
 */
