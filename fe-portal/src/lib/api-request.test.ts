...
import { sendApiRequest, type ErrorReporter } from "./api-request";
...
test("a request the backend never answers rejects at the deadline and is reported", async () => {
  ...
  await assert.rejects(
    sendApiRequest("http://api.test/x", { method: "GET" }, { report, label: "Portal API", timeoutMs: 20 }),
...
test("a 5xx is reported with its status and the backend's requestId", async () => {
  ...
  const result = await sendApiRequest("http://api.test/x", { method: "POST" }, { report, label: "Portal API" });
...
test("a 5xx without a JSON body is still reported, with no requestId", async () => {
  ...
  const result = await sendApiRequest("http://api.test/x", {}, { report, label: "Portal API" });
...
test("a caller-supplied signal wins over the default deadline", async () => {
  ...
  const pending = sendApiRequest(
    ...
    { report, label: "Portal API", timeoutMs: 10 },
  );
...
test("a caller's own abort is not reported as a failure", async () => {
  ...
  const pending = sendApiRequest("http://api.test/x", { signal: controller.signal }, { report, label: "Portal API" });
  controller.abort();
