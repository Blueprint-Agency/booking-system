...
export default function TenantSessionsPage() {
  ...
  const tenantSessions = useMemo(() => {
    if (!tenant) return [];
    return typedSessions.filter((s) => s.tenantId === tenant.id);
  }, [tenant]);

  // Derive categories from tenant's sessions
  const categories = useMemo(() => {
    const cats = Array.from(new Set(tenantSessions.map((s) => s.category)));
    return ["All", ...cats.sort()];
  }, [tenantSessions]);

  // Build category color map dynamically (extend the default with any new ones)
  const categoryColorMap = useMemo(() => {
    const map: Record<string, string> = { ...CATEGORY_COLORS };
    categories.forEach((cat) => {
      if (cat !== "All" && !map[cat]) {
        map[cat] = "bg-warm border-border text-ink";
      }
    });
    return map;
  }, [categories]);
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      ...
      {view === "calendar" && (
        ...
        >
            ...
            <div className="min-w-[800px]">
              ...
              <div className="relative">
                ...
                {filtered.map((session) => {
                  ...
                  const topOffset = (timeStart - 6) * 64;
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        ...
                        categoryColorMap[session.category] ||
          ...
          {/* Legend */}
          <div className="flex items-center gap-4 px-4 py-3 border-t border-border bg-warm/30 flex-wrap">
